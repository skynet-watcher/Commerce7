...
| Path | Purpose |
|------|---------|
| **`apps/api`** | Hono + TypeScript. **`PORT`** default **3001**. Loads **repo-root `.env`**. Webhook: `POST /webhooks/commerce7` (Zod + in-memory idempotency — see `docs/IMPLEMENTATION-LOG.md`). |
