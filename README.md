...
- **Run API + Integration UI together:** `pnpm dev:all` — or `pnpm dev:api` + `pnpm dev:web` in two terminals ([`docs/SANDBOX-TOMORROW.md`](docs/SANDBOX-TOMORROW.md)). **Winery marketing assistant** (merchant-facing checkup → recommendation → results): **http://localhost:3000/assistant** — add `?demo=1&winery=Name` for a client demo with seeded results. Detailed analytics report: **http://localhost:3000/dashboard** (see [`docs/IMPLEMENTATION-LOG.md`](docs/IMPLEMENTATION-LOG.md)).
